...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import titanlib_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("titanlib_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "titanlib_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"titanlib", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "titanlib_jll", "src", "titanlib_jll.jl"))
        @info("titanlib_ll dev'ed out to /tmp/bb/dev/titanlib_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("ee668f94-0f4f-5c2b-817e-1bb10fec470c"))
platforms = [Pkg.Artifacts.unpack_platform(e, "titanlib", artifacts_toml) for e in artifacts["titanlib"]]
...
if best_platform === nothing
    @debug("Unable to load titanlib; unsupported platform $(triplet(platform_key_abi()))")
else
